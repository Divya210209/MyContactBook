...
class ManageContactsFrame(Frame):
    def __init__(self, parent):
        ...
        s.configure('Treeview', font=('Arial', 14),rowheight=30)
    ...
    def create_view_all_contacts_frame(self):  
        self.view_all_contacts_frame = Frame(self, style='TFrame')
        self.view_all_contacts_frame.place(relx=.5, rely=.5, anchor=CENTER)

        add_new_contact_button = Button(self.view_all_contacts_frame, text="Add New Contact",
    ...
    def add_new_contact_button_click(self):
        ...
        email_id_label = Label(self.add_new_contact_frame, text="Email:")
        ...
        self.city_combobox = Combobox(self.add_new_contact_frame,width=28,font=('Arial',15),
        values=('hyd', 'jublee', 'chennai', 'mumbai', 'bangalore'))
        self.city_combobox.grid(row=3, column=1,pady=5)
        self.city_combobox.set("Select your city")
        
        add_button = Button(self.add_new_contact_frame, text="Add", width=30, command=self.add_button_click) #font=('Arial', 15)
    ...
    def contacts_treeview_row_selection(self,event):
        ...
        email_id_label = Label(self.update_delete_contact_frame, text="Email:")
        email_id_label.grid(row=2, column=0,sticky=E)
        ...
        self.city_combobox = Combobox(self.update_delete_contact_frame,width=28,font=('Arial',15),
        values=('hyd', 'jublee', 'chennai', 'mumbai', 'bangalore'))
        self.city_combobox.grid(row=3, column=1,pady=5)
        self.city_combobox.set(contact[3])
        
        update_button = Button(self.update_delete_contact_frame, text="Update", width=30,
        command=self.update_button_click) #font=('Arial', 15)
        update_button.grid(row=4, column=1,pady=5)

        delete_button = Button(self.update_delete_contact_frame, text="Delete", width=30) #font=('Arial', 15)
    ...
    def delete_button_click(self):
        if messagebox.askquestion("Confirmation Message","Are you sure to delete?")=='yes':
            ...
            self.con.commit()
            messagebox.showinfo("Success Message","Contacts details are deleted successfully")
        self.update_delete_contact_frame.destroy()
        self.create_view_all_contacts_frame()
...
if __name__ == '__main__':
    root = Tk()
    root.title("Manage Contacts")
    root.geometry("600x400")
    frame = ManageContactsFrame(root)
    root.mainloop()
